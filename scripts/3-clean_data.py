...
df_kobe_yao = (
  data["games_details"]
  .query("PLAYER_NAME.isin(['Kobe Bryant', 'Yao Ming'])")
  .copy()
)
...
df_kobe_yao = (
  data["games_details"]
  .query("PLAYER_NAME == 'Kobe Bryant' | PLAYER_NAME == 'Yao Ming'")
  .copy()
)
...
df_kobe_yao = (
  data["games_details"]
  .query("PLAYER_NAME.isin(@players)")
  .copy()
)
...
print((
  df_kobe_yao
  .groupby("PLAYER_NAME")[["PTS", "REB", "AST"]]
  .mean()
))
...
df_kobe_yao_final_filter = (
  df_kobe_yao_final
  .filter(
    items = 
    ["PLAYER_NAME", "GAME_DATE", "SEASON", "PTS", "MIN", "PTS_PER_MIN"]
  )
  .copy()
)
...
df_kobe_yao_final_filter_lower = (
  df_kobe_yao_final_filter
...
df_kobe_yao_final_filter_lower_nona = (
  df_kobe_yao_final_filter_lower
  .fillna(0)
  .copy()
)
...
df_kobe_yao_final_filter_lower_sort = (
  df_kobe_yao_final_filter_lower_nona
  .sort_values(
  by="pts_per_min", 
  ascending=False
